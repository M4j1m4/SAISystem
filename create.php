...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $email = $_POST['email'];  // Changed from 'gmail' to 'email'

    do {
        if (empty($username) || empty($category) || empty($password) || empty($email)) {
        ...
        $sql = "INSERT INTO users (username, category, password, gmail) VALUES (?, ?, ?, ?)";
        ...
        // Bind parameters
        $stmt->bind_param("ssss", $username, $category, $hashedPassword, $email);

        // Execute the statement
        if (!$stmt->execute()) {
            $errorMessage = "Failed to execute statement: " . $stmt->error;
            break;
        }
